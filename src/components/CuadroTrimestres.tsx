    ...
    <Card className={`shadow-xl border overflow-hidden mt-4 ${darkMode ? "bg-slate-950/40 backdrop-blur-md border-white/5 shadow-2xl text-white" : "bg-white border-slate-200"}`}>
      <CardContent className="p-3">
        <div className="flex flex-wrap items-end gap-3 mb-3">
          ...
          {asignaturaCuadroId && (
            <>
              <Button size="sm" variant="outline" onClick={exportarCuadroCSV} className={`h-11 sm:h-12 text-sm ${darkMode ? "bg-card border-white/30 text-white hover-gradient-strong" : ""}`}>
                <Download className="h-4 w-4 sm:h-5 sm:w-5 sm:mr-1" /><span className="hidden sm:inline">Excel (CSV)</span>
              </Button>
              <Button size="sm" variant="outline" onClick={exportarCuadroPDF} className={`h-11 sm:h-12 text-sm ${darkMode ? "bg-card border-white/30 text-white hover-gradient-strong" : ""}`}>
