...
<body>
  ...
  const handleCleanDuplicates = async () => {
    if (!confirm("¿Estás seguro de que deseas eliminar registros duplicados de asistencia? Esta acción revisará toda la base de datos y eliminará duplicados manteniendo solo el registro más reciente de cada estudiante por fecha.")) return;
        ...
        {view === "pass" ? (
          <>
            <div className={`flex flex-wrap items-end gap-2 sm:gap-3 p-2 sm:p-3 rounded-xl border ${darkMode ? 'bg-slate-800/50 border-slate-700' : 'bg-slate-50'}`}>
              ...
              <Button
                variant="outline"
                className={`h-10 w-full sm:w-auto px-4 font-bold text-xs sm:text-sm ${darkMode ? 'border-purple-700 text-purple-400 hover:bg-purple-900/30' : 'border-purple-200 text-purple-700 hover:bg-purple-50'}`}
