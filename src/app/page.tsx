...
import { 
  ...
  Download, Trash2, ListPlus, UserPlus, Key, Calendar, LayoutDashboard, CalendarDays, GripVertical
} from "lucide-react";
...
export default function Home() {
  ...
  const handleDragEnd = (event: DragEndEvent) => {
    ...
    if (!over || active.id === over.id || !usuario || usuario.rol !== "admin") return;
              ...
              <CardContent className="pt-0">
                ...
                <div className={`rounded border ${darkMode ? 'border-slate-700' : ''}`}>
                  {usuario.rol === "admin" ? (
                    <DndContext collisionDetection={closestCenter} onDragEnd={handleDragEnd}>
                      <SortableContext items={estudiantes.map(e => e.id)} strategy={verticalListSortingStrategy}>
                        <Table className="text-sm sm:text-base font-medium">
                          <TableHeader><TableRow className={darkMode ? 'bg-slate-800' : 'bg-slate-100'}><TableHead className="w-10 text-center h-12">N°</TableHead><TableHead>Nombre Completo</TableHead><TableHead className="w-16 text-center">Estado</TableHead><TableHead className="w-12 text-center">Orden</TableHead><TableHead className="w-12 text-center">Acciones</TableHead></TableRow></TableHeader>
                          <TableBody>
                            {!estudiantes || estudiantes.length === 0 ? <TableRow><TableCell colSpan={5} className={`text-center py-8 ${darkMode ? 'text-slate-500' : 'text-slate-400'}`}>No hay estudiantes</TableCell></TableRow> :
                              estudiantes.map((est, idx) => <SortableEstudianteRow key={est.id} est={est} idx={idx} estudiantes={estudiantes} darkMode={darkMode} onDelete={() => handleDeleteEstudiante(est.id, est.nombre)} onReorder={handleReordenarEstudiantes} />)}
                          </TableBody>
                        </Table>
                      </SortableContext>
                    </DndContext>
                    ...
                    <Table className="text-sm sm:text-base font-medium">
                      <TableHeader><TableRow className={darkMode ? 'bg-slate-800' : 'bg-slate-100'}><TableHead className="w-10 text-center h-12">N°</TableHead><TableHead>Nombre Completo</TableHead><TableHead className="w-16 text-center">Estado</TableHead></TableRow></TableHeader>
                      <TableBody>
                        {!estudiantes || estudiantes.length === 0 ? <TableRow><TableCell colSpan={3} className={`text-center py-8 ${darkMode ? 'text-slate-500' : 'text-slate-400'}`}>No hay estudiantes</TableCell></TableRow> :
...
const SortableEstudianteRow = ({ est, idx, estudiantes, darkMode, onDelete, onReorder }: SortableEstudianteRowProps) => {
  const { attributes, listeners, setNodeRef, transform, transition, isDragging } = useSortable({ id: est.id });
  const style = { transform: CSS.Transform.toString(transform), transition, opacity: isDragging ? 0.5 : 1 };
  const moverArriba = () => { if (idx > 0) { const nuevos = [...estudiantes]; const temp = nuevos[idx]; nuevos[idx] = nuevos[idx - 1]; nuevos[idx - 1] = temp; onReorder(nuevos); } };
  const moverAbajo = () => { if (idx < estudiantes.length - 1) { const nuevos = [...estudiantes]; const temp = nuevos[idx]; nuevos[idx] = nuevos[idx + 1]; nuevos[idx + 1] = temp; onReorder(nuevos); } };
  return (
    <TableRow ref={setNodeRef} style={style} className={`${darkMode ? 'border-slate-700' : ''} ${isDragging ? 'bg-teal-900/30' : ''}`}>
      ...
      <TableCell className="text-center"><Badge variant={est.activo ? "default" : "secondary"} className={`text-xs sm:text-sm font-medium h-5 ${est.activo ? (darkMode ? 'bg-teal-600' : '') : ''}`}>{est.activo ? "Activo" : "Inactivo"}</Badge></TableCell>
      <TableCell className="text-center"><div className="flex items-center justify-center gap-1"><Button size="sm" variant="ghost" className={`h-6 w-6 p-0 ${idx === 0 ? 'opacity-30 cursor-not-allowed' : ''} ${darkMode ? 'text-slate-400 hover:text-teal-400' : 'text-slate-500 hover:text-teal-600'}`} onClick={moverArriba} disabled={idx === 0}><ChevronUp className="h-4 w-4" /></Button><Button size="sm" variant="ghost" className={`h-6 w-6 p-0 ${idx === estudiantes.length - 1 ? 'opacity-30 cursor-not-allowed' : ''} ${darkMode ? 'text-slate-400 hover:text-teal-400' : 'text-slate-500 hover:text-teal-600'}`} onClick={moverAbajo} disabled={idx === estudiantes.length - 1}><ChevronDown className="h-4 w-4" /></Button></div></TableCell>
      <TableCell className="text-center"><Button size="sm" variant="ghost" className={`h-6 w-6 p-0 ${darkMode ? 'text-red-400 hover:text-red-300 hover:bg-red-900/30' : 'text-red-500 hover:text-red-700 hover:bg-red-50'}`} onClick={onDelete}><Trash2 className="h-5 w-5" /></Button></TableCell>
    </TableRow>
  );
};
