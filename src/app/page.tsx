  ...
  if (!config) {
    return (
      <tr className={`border-b transition-colors ${rowBg}`}>
        ...
        <td className={`p-2 text-center border-l ${cellBorder} ${finalBg}`}><span className={`inline-block px-2 py-0.5 rounded-md text-xs sm:text-sm font-bold shadow ${finalBadgeClass}`}>{promFinal !== null ? Math.round(promFinal).toString() : "-"}</span></td>
