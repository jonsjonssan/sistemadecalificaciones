...
export async function POST(request: NextRequest) {
  try {
    ...
    // Grado info para audit y realtime
    let gradoInfo: { numero: number; seccion: string } | null = null;
    if (session?.id && finalResult?.estudiante?.gradoId) {
      gradoInfo = await db.grado.findUnique({ where: { id: finalResult.estudiante.gradoId }, select: { numero: true, seccion: true } });
    }
    ...
    emitRealtimeEventAsync({
      ...
      payload: {
        ...
        accion: "UPDATE",
        entidad: "Calificacion",
        usuario: { nombre: session.nombre, email: session.email, rol: session.rol },
        detalles: {
          estudiante: finalResult?.estudiante?.nombre,
          materia: finalResult?.materia?.nombre,
          trimestre: parseInt(String(trimestre)),
