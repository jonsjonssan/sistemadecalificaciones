                      ...
                      <table className="w-full text-sm sm:text-base font-medium border-collapse">
                        ...
                        <tbody>
                          {sectionLoading ? (
                            ...
                            getFilteredAndSortedStudents().map((est, idx) => {
                              const calif = calificaciones.find(c => c.estudianteId === est.id);
              ...
              <TabsContent value="admin" className="mt-3 space-y-3 sm:space-y-4">
                ...
                <Card className={`shadow-sm ${darkMode ? 'bg-[#1e293b] border-slate-700' : ''}`}>
                  ...
                  <CardContent className="pt-0 space-y-4">
                    {/* Usuarios Conectados */}
                    <div>
                      <div className={`flex items-center gap-2 mb-2 ${darkMode ? 'text-slate-300' : 'text-slate-700'}`}>
                        <div className="h-2 w-2 rounded-full bg-green-500 animate-pulse" />
                        <span className="text-xs font-medium">Usuarios Conectados ({usuariosConectados.length})</span>
                      </div>
                      <div className={`rounded border overflow-x-auto max-h-40 ${darkMode ? 'border-slate-700' : ''}`}>
                        {usuariosConectados.length === 0 ? (
                          <div className={`p-3 text-xs text-center ${darkMode ? 'text-slate-500' : 'text-slate-500'}`}>No hay usuarios conectados</div>
                        ) : (
                          <table className="w-full text-xs">
                            <thead><tr className={darkMode ? 'bg-slate-800' : 'bg-slate-100'}>
                              <th className="p-2 text-left">Usuario</th>
                              <th className="p-2 text-left">Rol</th>
                              <th className="p-2 text-left">Hora de conexión</th>
                            </tr></thead>
                            <tbody>
                              {usuariosConectados.map((u: any) => (
                                <tr key={u.id} className={`border-t ${darkMode ? 'border-slate-700' : ''}`}>
                                  <td className="p-2">{u.usuario?.nombre || 'Desconocido'}</td>
                                  <td className="p-2"><Badge variant="secondary" className="text-[10px]">{u.usuario?.rol === 'admin' ? 'Admin' : u.usuario?.rol === 'admin-directora' ? 'Directora' : u.usuario?.rol === 'admin-codirectora' ? 'Codirectora' : u.usuario?.rol === 'docente-orientador' ? 'Doc-Orient' : 'Docente'}</Badge></td>
                                  <td className="p-2">{u.loginAt ? new Date(u.loginAt).toLocaleTimeString('es-DO') : '—'}</td>
                                </tr>
                              ))}
                            </tbody>
                          </table>
                    ...
                    <div>
                      <div className={`flex items-center gap-2 mb-2 ${darkMode ? 'text-slate-300' : 'text-slate-700'}`}>
                        ...
                        <span className="text-xs font-medium">Actividad Reciente ({actividadReciente.length})</span>
                      </div>
                      <div className={`rounded border overflow-x-auto max-h-48 ${darkMode ? 'border-slate-700' : ''}`}>
                        {actividadReciente.length === 0 ? (
                          <div className={`p-3 text-xs text-center ${darkMode ? 'text-slate-500' : 'text-slate-500'}`}>Sin actividad reciente</div>
                        ) : (
                          <table className="w-full text-xs">
                            <thead><tr className={darkMode ? 'bg-slate-800' : 'bg-slate-100'}>
                            ...
                            <tbody>
                              {actividadReciente.map((a: any) => (
                                <tr key={a.id} className={`border-t ${darkMode ? 'border-slate-700' : ''}`}>
                                  <td className="p-2 whitespace-nowrap">{a.fecha ? new Date(a.fecha).toLocaleTimeString('es-DO') : '—'}</td>
                                  <td className="p-2">{a.usuario?.nombre || 'Desconocido'}</td>
                                  <td className="p-2"><Badge variant={a.accion === 'DELETE' ? 'destructive' : 'default'} className={`text-[10px] ${a.accion === 'DELETE' ? '' : (darkMode ? 'bg-teal-600' : 'bg-teal-600')}`}>{a.accion === 'DELETE' ? 'Borró' : 'Digitó'}</Badge></td>
                                  <td className="p-2"><Badge variant="outline" className="text-[10px]">{a.tipo}</Badge></td>
                                  <td className="p-2 max-w-[200px] truncate">{a.detalles ? (a.detalles.estudiante || a.detalles.materia || a.detalles.grado || '—') : '—'}</td>
                                </tr>
                              ))}
                            </tbody>
                          </table>
